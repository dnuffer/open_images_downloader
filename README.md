...
The application is written in scala and depends on the Java jre to run. It is distributed with a shell script and batch file generated by sbt-native-packager (http://www.scala-sbt.org/sbt-native-packager/index.html), so all you need to run it is to execute open\_images\_downloader (or the .bat on windows).
...
* optionally validate the image is a proper jpeg
    ...
    * There is a list of running my detect\_corrupt\_jpeg script at:
...
* resume downloading partially downloaded files? Seems a bit pointless, none of the files are that big. Would be interesting to implement anyway! The file images\_2017_07.tar.gz is the largest.
